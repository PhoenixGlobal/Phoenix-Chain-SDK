...
import (
	...
	imath "github.com/PhoenixGlobal/Phoenix-Chain-SDK/libs/common/math"
...
func Transfer(proc *exec.Process, dst uint32, amount uint32, len uint32) int32 {
	...
	gasTemp, err := callGas(ctx.contract.Gas, configs.TxGas, uint256.NewInt(0).SetUint64(ctx.contract.Gas))
	if nil != err {
		panic(err)
	}
